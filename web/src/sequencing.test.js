...
// A sequenced incremental event (phase_changed) and a full snapshot
// (state_snapshot) are the two sequenced shapes the sequencer gates;
// chat_broadcast stands in for the unsequenced case.
const phase = (seq) => ({ type: T.PhaseChanged, seq })
...
describe('decideSequence', () => {
  ...
  it('applies a sequenced event with no seq rather than crashing on it', () => {
    expect(decideSequence(5, phase(undefined))).toBe('apply')
  })

  it('applies the very first sequenced message seen, with no baseline yet', () => {
    expect(decideSequence(null, phase(7))).toBe('apply')
  })

  it('applies exactly the next expected revision', () => {
    expect(decideSequence(5, phase(6))).toBe('apply')
  })

  it('ignores an exact duplicate', () => {
    expect(decideSequence(5, phase(5))).toBe('duplicate-or-old')
  })

  it('ignores an event older than what is already applied', () => {
    expect(decideSequence(10, phase(3))).toBe('duplicate-or-old')
  })

  it('requests a resync on a sequence gap, without applying it', () => {
    expect(decideSequence(5, phase(8))).toBe('gap')
  })
