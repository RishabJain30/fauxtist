...
// ICE/TURN credential minting: the endpoint returns configured STUN + a fresh
// time-limited TURN credential over an established session, STUN-only works
// without TURN, credentials are correct HMAC-SHA1s that change over time, and
// the shared secret itself never appears in a response.
